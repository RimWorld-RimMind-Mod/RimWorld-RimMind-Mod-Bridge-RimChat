...
- **避免重复对话**：当 RimChat 激活时，自动跳过 RimMind 的重复对话触发（闲聊、自动对话、玩家对话三种类型独立控制）
...
| 模组 | 职责 | 依赖 | GitHub |
|------|------|------|--------|
| RimMind-Core | 公共 API、LLM 请求调度、4-Zone 上下文引擎、ToolCall 与运行时 | Harmony | [链接](https://github.com/RimWorld-RimMind-Mod/RimWorld-RimMind-Mod-Core) |
...
| RimMind-Advisor | 状态/Thought → 建议、玩家审批、动作与反馈周期 | Core | [链接](https://github.com/RimWorld-RimMind-Mod/RimWorld-RimMind-Mod-Advisor) |
| RimMind-Dialogue | AI 驱动对话与社交关系演进（express_dialogue） | Core | [链接](https://github.com/RimWorld-RimMind-Mod/RimWorld-RimMind-Mod-Dialogue) |
...
当 RimChat 激活时，自动跳过 RimMind-Dialogue 的重复对话触发，三种对话类型独立控制：

- **启用闲聊门控**：跳过 RimMind 的闲聊触发，避免与 RimChat 重复对话（默认开启）
- **启用自动对话门控**：跳过 RimMind 的自动对话触发，避免与 RimChat 重复对话（默认开启）
- **启用玩家对话门控**：跳过 RimMind 的玩家对话触发并移除"与X对话"浮动菜单选项（默认开启）
...
| 设置 | 默认值 | 说明 |
|------|--------|------|
| 启用闲聊门控 | 开启 | RimChat 激活时跳过 RimMind 闲聊触发 |
| 启用自动对话门控 | 开启 | RimChat 激活时跳过 RimMind 自动对话触发 |
| 启用玩家对话门控 | 开启 | RimChat 激活时跳过 RimMind 玩家对话触发 |
...
**Q: 闲聊门控和自动对话门控有什么区别？**
A: "闲聊门控"控制 RimMind-Dialogue 的 Chitchat 触发（殖民者之间的闲聊），"自动对话门控"控制 Auto 触发（AI 自主发起的对话）。两者独立控制，可以分别开关。
...
- **Prevent duplicate dialogues**: Automatically skip RimMind's duplicate dialogue triggers when RimChat is active (chitchat, auto dialogue, and player dialogue independently controlled)
...
When RimChat is active, automatically skips RimMind-Dialogue's duplicate dialogue triggers. Three dialogue types are independently controlled:

- **Gate chitchat triggers**: Skip RimMind's chitchat triggers to avoid duplicate conversations with RimChat (default: on)
- **Gate auto dialogue**: Skip RimMind's auto dialogue triggers to avoid duplicate conversations with RimChat (default: on)
- **Gate player dialogue**: Skip RimMind's player dialogue triggers and remove the "Chat with X" float menu option (default: on)
...
|---------|---------|-------------|
| Gate chitchat triggers | On | Skip RimMind chitchat triggers when RimChat is active |
| Gate auto dialogue | On | Skip RimMind auto dialogue triggers when RimChat is active |
| Gate player dialogue | On | Skip RimMind player dialogue triggers when RimChat is active |
...
**Q: What's the difference between "Gate chitchat triggers" and "Gate auto dialogue"?**
A: "Gate chitchat triggers" controls RimMind-Dialogue's Chitchat triggers (casual conversations between colonists). "Gate auto dialogue" controls Auto triggers (AI-initiated dialogues). They are independently controlled.
